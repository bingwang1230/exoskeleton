...
> 最后更新：2026-09-16（第三章 tendon 回炉：审查题定位层过、账本层用户自评未懂；学习方式=审查者模式）
...
| 执行器绑 tendon 后 ctrl 单位变张力（力矩=张力×半径） | 0.4（09-16 降档：追问「张力是哪里的张力」暴露 ctrl=指令张力未接住；此前 0.6 判卷过急） | 同上 |
| 关节限位=约束力（qfrc_constraint/efc 限位）：停转位置由 range 定、门框受压由张力定 | 0.3（09-16 降档：range 对位是用户自己算的，但机制/账本层未懂，自评未过） | 同上 |
| 审查者模式 6 条清单 | 0.3 | 同上 |

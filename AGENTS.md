...
| `content/` | 内容素材台账（INDEX + 文本条目；原始录屏/照片不进 git），纪律见 §10 |
...
- **位置与形态**：`content/INDEX.md` 为唯一台账，两区——「待拍摄」（提醒用）与「已入库」（文本/指针）。文本素材 AI 直接写成条目（短的入 INDEX，长的独立 `content/YYYYMMDD-短名.md`）；录屏/照片等原始文件**不进 git**，条目只记状态与存放路径。commit 前缀 `content:`。
